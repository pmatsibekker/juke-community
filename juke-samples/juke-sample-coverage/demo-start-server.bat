...
::
::  Optional: in a SECOND window run demo-run-playwright.bat to drive the
...
if not exist "%JAR%" (
    ...
    echo   mvn -pl juke-samples/juke-sample-coverage -am package -Pcoverage -DskipTests
    echo.
...
if not exist "%USERPROFILE%\juke-demo" (
    ...
    echo Created %USERPROFILE%\juke-demo
)
if not exist "%USERPROFILE%\juke-demo\coverage" (
    mkdir "%USERPROFILE%\juke-demo\coverage"
)

:: ── Coverage args ─────────────────────────────────────────────────────────────
:: The JaCoCo agent jar is copied to target/ by Maven; we attach it with
:: output=none (juke-coverage reads execution data in-process via RT.getAgent()),
:: point juke.coverage at this module's classes/sources, and accept the
:: thresholds set in application.yml.
::
...
echo.
echo ======================================================
echo   Juke coverage demo -^> http://localhost:8080
echo   Recordings:   %USERPROFILE%\juke-demo\sample-coverage
...
java %COVERAGE_ARGS% "-Djuke.enabled=true" "-Djuke.path=%USERPROFILE%\juke-demo\sample-coverage" "-Djuke.zip=coverage-demo" -jar "%JAR%"
