...
Build it first (from the repo root):
  mvn -pl juke-samples/juke-sample-status-grid -am package -DskipTests
"@
    exit 1
}
...
$RecordDir = "$env:USERPROFILE\juke\sample-status-grid"
...
Write-Host "  Juke status-grid server -> http://localhost:8080" -ForegroundColor Yellow
Write-Host "  Recordings stored in: $RecordDir"                 -ForegroundColor Yellow
Write-Host "  Open http://localhost:8080 for the live grid UI." -ForegroundColor Yellow
